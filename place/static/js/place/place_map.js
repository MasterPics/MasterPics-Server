...
function displayContactMarkers(){
    ...
    for (let i = 0; i < places.length; i ++) {
        ...
        // 마커 위치
        let markerLoc = new kakao.maps.LatLng(places[i].lat, places[i].lon)
        ...
        let infowindow = new kakao.maps.InfoWindow({
            content: displayCustomInfoWindow(places[i]), // 인포윈도우에 표시할 내용
            zIndex: 3
        });
...
function displayCustomInfoWindow(place) {
    let content = '<div class="info-window">' +
        '<div class="title"> Title: ' + place.title + '</div>' +
        '<div class="pay"> Payment: ' + place.pay + '</div>' + 
        '<div class="address"> Address: ' + place.address + '</div>' + 
        '<a href="/place/detail/' + place.pk + '/">자세히보기</a>' +  
    '</div>'
...
function giveDetailListner(index) {
    return function() {
        window.location.href = "/place/detail/" + (index + 1) + "/";
    }
}
